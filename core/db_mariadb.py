"""MariaDB / MySQL backend for the relay server.
...
message. The full implementation (pymysql driver, ``%s`` placeholders,
backend-specific schema + migrations) is tracked separately.
...
``create_database()`` wires to this class when ``settings.db_type == "mariadb"``.
"""
...
class MariadbDatabase(Database):
    """MariaDB / MySQL backend — pymysql-based. Not yet implemented."""
    ...
    def get_conn(self) -> Any:
        raise NotImplementedError(
            "MariaDB support is not yet implemented. "
            "The Database interface is in place; the pymysql-backed driver, "
